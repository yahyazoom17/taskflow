...
export function KanbanBoard({ initialTasks }: KanbanBoardProps) {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (active.id !== over.id) {
      ...
      // Update state first, then persist outside the render cycle
      setTasks(updated);
      reorderTasks(updated).catch(() => toast.error("Failed to save changes"));
    } else {
      // Same position but might have changed columns (via dragOver)
      ...
      <main className="flex-1 overflow-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          {mounted ? (
            ...
            /* Static placeholder during SSR/hydration — matches layout but has no dnd-kit attributes */
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {COLUMNS.map((col) => (
